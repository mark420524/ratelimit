package com.siival.ratelimit.util;

import com.siival.ratelimit.annotation.CheckTypeEnum;
...
public class RateLimiterUtil {
     ...
     * @return key
     */
    public static String getRateKey(JoinPoint joinPoint, CheckTypeEnum checkTypeEnum,String name){
        ...
        }else if(CheckTypeEnum.IP.equals(checkTypeEnum)){
            //以IP地址作为key
            key.append(getIpAddr(request));
        }else if (CheckTypeEnum.MIP.equals(checkTypeEnum)) {
            key.append(name);
            key.append(getIpAddr(request));
        }else  if(CheckTypeEnum.CUSTOM.equals(checkTypeEnum)){
            //以自定义内容作为key
            if(request.getAttribute(Const.CUSTOM)!=null){
                key.append(request.getAttribute(Const.CUSTOM).toString());
            }else{
                throw new BusinessException(BusinessErrorEnum.CUSTOM_NOT_FOUND);
            }
        }
        return key.toString();
    }
